...
const FIXED_PERSISTENCE: float = 0.9
...
func _on_lod_timer_timeout() -> void:
	...
	var radius: float = $StaticBody3D/CollisionShape3D.shape.radius
...
func _generate_planet_material() -> void:
	...
	planet_material.set_shader_parameter("color_deep", Color.from_hsv(base_hue, rng.randf_range(0.5, 0.9), rng.randf_range(0.1, 0.3)))
